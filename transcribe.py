...
class WhisperModel:
    def __init__(
        ...
        if os.path.isdir(model_size_or_path):
            ...
            model_path = download_model(
                ...
                output_dir="/workspace/faster-whisper/models",
            )
    ...
    def _get_feature_kwargs(self, model_path) -> dict:
        ...
        if os.path.isfile(preprocessor_config_file):
            try:
                ...
                self.logger.warning("Could not load preprocessor_config.json: %s", str(e))
    ...
    def generate_segments(
        ...
        while seek < content_frames:
            ...
            single_timestamp_ending = (
                len(tokens) >= 2 and tokens[-2] < tokenizer.timestamp_begin and tokens[-1] >= tokenizer.timestamp_begin
            )
    ...
    def generate_with_fallback(
        ...
        for temperature in options.temperatures:
            ...
            if options.no_speech_threshold is not None and result.no_speech_prob > options.no_speech_threshold:
    ...
    def add_word_timestamps(
        ...
        alignment = self.find_alignment(tokenizer, text_tokens, encoder_output, num_frames)
        ...
        time_offset = segments[0]["seek"] * self.feature_extractor.hop_length / self.feature_extractor.sampling_rate
    ...
    def find_alignment(
        ...
        return [
            dict(word=word, tokens=tokens, start=start, end=end, probability=probability)
